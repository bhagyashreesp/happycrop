...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $product_id = NULL, $exclude_seller = NULL)
{
    ...
    $t->db->select($discount_filter_data . ' (select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type ,
     p.is_prices_inclusive_tax, p.type, p.product_stock_status ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids,sd.rating as seller_rating,sd.slug as seller_slug,sd.no_of_ratings as seller_no_of_ratings,sd.logo as seller_profile, sd.store_name as store_name,sd.store_description, sd.company_name, p.seller_id, u.username as seller_name,
     p.id,p.stock,p.name,p.category_id,p.short_description,p.slug, p.super_category_id,p.description,p.rec_crops,p.about_company,p.about_formulation,p.about_usage,p.method_of_app,p.dosage,p.specifications,p.benefits,p.total_allowed_quantity,p.deliverable_type,p.deliverable_zipcodes,p.minimum_order_quantity,
     ...
     p.video_type, p.video, p.tags, p.warranty_period, p.guarantee_period, p.made_in,p.availability,c.name as category_name,tax.percentage as tax_percentage, pv.mrp as _mrp, pv.price as _price, pv.special_price as _special_price, pv.mrp_per_item as _mrp_per_item, pv.price_per_item as _price_per_item, pv.special_price_per_item as _special_price_per_item, sb.brand_name, sd.about_us as seller_about_us, sc.name as super_category_name, sc.is_service_category, p.tools, p.technician, p.certifications, p.pro_no, p.serv_no ')
        ...
        ->join('`seller_brands` sb', ' sb.id = p.brand_id ', 'LEFT')
        ->join('`super_categories` sc', ' sc.id = p.super_category_id ', 'LEFT');
    ...
    if(isset($filter) && !empty($filter['seller_id']))
    {
        $where['p.seller_id'] = $filter['seller_id'];
    }
    ...
    if(isset($filter['target_insect_id']) && !empty($filter['target_insect_id']) && $filter['target_insect_id']!='')
    {
        if(is_array($filter['target_insect_id']))
        {
            //
            $t->db->group_Start();
            foreach($filter['target_insect_id'] as $target_insect_id)
            {
                $target_insect_sql = "FIND_IN_SET('".$target_insect_id."', p.target_insect_id)";
                $t->db->or_where($target_insect_sql);
            }
            $t->db->group_End();
        }
        else
        {
            $target_insect_sql = "FIND_IN_SET('".$filter['target_insect_id']."', p.target_insect_id)";
            $t->db->where($target_insect_sql);
        }
    }
    
    if(isset($product_id) && !empty($product_id) && $product_id != "")
    {
        $t->db->where('p.id !=', $product_id, false);
    }
    
    if(isset($exclude_seller) && !empty($exclude_seller))
    {
        $t->db->where('p.seller_id !=', $exclude_seller, false);
    }
    ...
    if (isset($category_id) && !empty($category_id)) {
        if (is_array($category_id) && !empty($category_id)) {
            ...
            //$t->db->or_where('p.category_id',$category_id);
            //$t->db->or_where('p.parent_id',$category_id);
            //$t->db->or_where('p.formulation_id',$category_id);
            $t->db->where('(p.category_id ='.$category_id.' OR p.parent_id ='.$category_id.' OR p.formulation_id  ='.$category_id.')');
        }
    }
    ...
    if(isset($exclude_seller) && !empty($exclude_seller))
    {
        $product_count->where('p.seller_id !=', $exclude_seller, false);
    }
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            foreach ($attributes_array as $attribute) {
                ...
                if (($key === 0 || !empty($key)) && isset($product[0]['attributes'][$key])) {
                    ...
                    $t->db->select('id');
                    $t->db->where_in('product_id', $product[0]['id']);
                    $t->db->where_in('attribute_value_ids', explode(',', $product[0]['attributes'][$key]['ids']));
                    $pro_variant_ids = $t->db->get('product_variants')->result_array();
                    $pro_variant_ids = array_column($pro_variant_ids, "id");
                    ...
                    $variant_attributes[$key]['volume'] = $product[0]['attributes'][$key]['volume'];
                    $variant_attributes[$key]['unit'] = $product[0]['attributes'][$key]['unit'];
                    $variant_attributes[$key]['unit_id'] = $product[0]['attributes'][$key]['unit_id'];
                    $variant_attributes[$key]['variant_ids'] = $pro_variant_ids;
...
function get_attribute_values_by_pid($id)
{
    ...
    $attribute_values = $t->db->select(" group_concat(`av`.`id`) as ids,group_concat(' ',`av`.`value`) as value ,`a`.`name` as attr_name, a.name, GROUP_CONCAT(av.swatche_type ORDER BY av.id ASC ) as swatche_type , GROUP_CONCAT(av.swatche_value  ) as swatche_value , GROUP_CONCAT(av.volume  ) as volume , GROUP_CONCAT(av.unit_id  ) as unit_id , GROUP_CONCAT(un.unit  ) as unit")
        ->join('attribute_values av ', 'FIND_IN_SET(av.id, pa.attribute_value_ids ) > 0', 'inner')
        ->join('attributes a', 'a.id = av.attribute_id', 'inner')
        ->join('units un', 'un.id = av.unit_id', 'inner')
    ...
    if (!empty($attribute_values)) {
        for ($i = 0; $i < count($attribute_values); $i++) {
            ...
            $volumes = array();
            $units = array();
            $volumes =  explode(",", $attribute_values[$i]['volume']);
            $units =  explode(",", $attribute_values[$i]['unit']);
...
function validate_order_status($order_ids, $status, $table = 'order_items', $user_id = null)
{
    ...
    $cancelable_count = 0;
    $return_request = 0;
    $check_status = ['received', 'qty_update', 'qty_approved', 'qty_rejected','payment_demand','payment_ack', 'schedule_delivery', 'processed', 'shipped', 'delivered', 'cancelled', 'returned'];
    ...
    if (in_array(strtolower(trim($status)), $check_status)) {
        ...
        for ($i = 0; $i < count($product_data); $i++) {
            ...
            if ($is_posted_status_set_count == count($product_data)){ //} && ($status !='qty_update' || $status !='qty_approved' || $status !='payment_demand' || $status!='schedule_delivery')) {
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '')
{
    ...
    //$t->db->join('unit_sizes usz', 'p.unit_size_id = usz.id', 'left');
    $t->db->join('units uu', 'pv.unit_id = uu.id', 'left');
    $t->db->join('super_categories sc', 'sc.id = p.super_category_id', 'left');
    $t->db->where(['p.status' => '1', 'pv.status' => 1, 'sd.status' => 1]);
    $t->db->group_by('c.id')->order_by('c.id', "DESC");
    $data = $t->db->get('cart c')->result_array();
    $total = $mrp_total = array();
    $variant_id = array();
    $quantity = array();
    $percentage = array();
    $amount = array();
    ...
    for ($i = 0; $i < count($data); $i++) {
        ...
        $standard_price_total[$i] = floatval($data[$i]['price']) * $data[$i]['qty'];
        $mrp_total[$i] = floatval($data[$i]['mrp']) * $data[$i]['qty'];
        ...
        $data[$i]['standard_price'] = $data[$i]['price'];
        ...
        $data[$i]['is_service_category'] = $data[$i]['is_service_category'];
    }
    $total = array_sum($total);
    $mrp_total = array_sum($mrp_total);
    $standard_price_total = array_sum($standard_price_total);
    ...
    $data['total_mrp'] = strval($mrp_total);
    $data['total_standard_price'] = strval($standard_price_total);
...
function send_mail2($to, $subject, $message)
{
    ...
    $config = array(
        ...
        'smtp_host' => 'localhost',
        'smtp_port' => '25',
        'smtp_user' => 'donotreply@happycrop.in',
        //'smtp_crypto' => 'TLS',
        //'smtp_host' => 'smtpout.secureserver.net',
        //'smtp_port' => '587',
        //'smtp_pass' => 'do@notry2023#',//'donotreply2023#',
        //'smtp_pass' => '7AcPn9ATY5JB',
        'charset' => 'utf-8',
        //'starttls'=>TRUE,
        //'_smtp_auth'=>TRUE,
        //'newline' => '\r\n',
    ...
    $t->email->from($config['smtp_user'], 'Happycrop');
    $t->email->to($to);
...
function send_mail3($to, $subject, $message)
{
    $t = &get_instance();
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null)
{
    ...
    $count_res = $t->db->select(' COUNT(distinct o.id) as `total`')
        ...
        ->join('units uu', 'pv.unit_id = uu.id', 'left')
        ->join('`taxes` tax', 'tax.id = p.tax', 'LEFT');
    ...
    $search_res = $t->db->select(' o.*, u.username,u.country_code, p.name, p.slug, oi.mrp, s.username as seller_name, sd.company_name, s.id as seller_id, pv.packing_size as pv_packing_size, uu.unit as pv_unit, pv.carton_qty as pv_carton_qty, tax.percentage as tax_percentage, rd.company_name as retailer_company_name, rd.gst_no as retailer_gst_no, sd.gst_no as seller_gst_no, s.mobile as seller_mobile, uuu.unit as unit')
        ->join(' `users` u', 'u.id= o.user_id', 'left')
        ->join(' `order_items` oi', 'o.id= oi.order_id', 'left')
        ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
        ->join('addresses a', 'a.id=o.address_id', 'left')
    ...
    //echo $t->db->last_query();die;
    
    for ($i = 0; $i < count($order_details); $i++) {
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,sd.store_name ,sd.store_description,sd.rating as seller_rating,sd.logo as seller_profile,ot.courier_agency,ot.tracking_id,ot.url,u.username as seller_name,p.is_returnable,p.image,p.name,p.slug,p.type,oi.status as status,(Select count(id) from order_items where order_id = oi.order_id ) as order_counter ,(Select count(active_status) from order_items where active_status ="cancelled" and order_id = oi.order_id ) as order_cancel_counter , (Select count(active_status) from order_items where active_status ="returned" and order_id = oi.order_id ) as order_return_counter, sd.company_name, p.licence_no, pv.packing_size as pv_packing_size, uu.unit as pv_unit, pv.carton_qty as pv_carton_qty, p.minimum_order_quantity, tax.percentage as tax_percentage, uuu.unit as unit ')
            ->join('product_variants pv', 'pv.id=oi.product_variant_id', 'left')
            ->join('products p', 'pv.product_id=p.id', 'left')
...
function get_min_max_price_of_product($product_id = '')
{
    ...
    $response = $t->db->select('is_prices_inclusive_tax,mrp,price,special_price,tax.percentage as tax_percentage')->get('products p')->result_array();
    ...
    $data['mrp'] = min(array_column($response, 'mrp'));
...
function get_default_price($product_id = '')
{
    $system_settings = get_settings('system_settings', true);
...
function get_insect_pest_id_by_slug($slug)
{
    $t = &get_instance();
    $slug = urldecode($slug);
    return $t->db->select("id")
        ->where('slug', $slug)
        ->get('insect_pests')->row_array()['id'];
}
...
function get_favorites($user_id, $limit = NULL, $offset = NULL)
{
    ...
    $res = $CI->db->join('products p', 'p.id=f.product_id')
        ...
        ->select('p.*, sd.company_name as seller_name, sd.slug as seller_slug')
        ->order_by('f.id', "DESC")
...
function get_sliders($id = '', $type = '', $type_id = '', $slider_type_ids = '')
{
    $ci = &get_instance();
    if (!empty($id)) {
        $ci->db->where('id', $id);
    ...
    if($slider_type_ids!='')
    {
        $ci->db->where("find_in_set($slider_type_ids, slider_type_ids)");
    }
...
function retailer_orders_total($user_id = "")
{
    $t = &get_instance();
    $t->db->select('sum(`final_total`) total');
    
    if(!empty($user_id)) {
        $t->db->where('user_id', $user_id);
    }
    
    $result = $t->db->from("orders")->get()->row_array();
    
    return $result['total'];
}

function retailer_orders_count($status = "", $user_id = "")
{
    $t = &get_instance();
    ...
    if(!empty($status)) {
        if(is_array($status))
        {
            $t->db->where_in('order_status', $status);
        }
        else
        {
            $t->db->where('order_status', $status);
        }
    }
...
function retailer_wish_count($user_id = "")
{
    $t = &get_instance();
    $t->db->select('count(DISTINCT `id`) total');
    ...
    $result = $t->db->from("favorites")->get()->row_array();
    
    return $result['total'];
}


function admin_orders_total()
{
    $t = &get_instance();
    $t->db->select('sum(a.`final_total`) total');
    
    $t->db->join('order_items as b','a.id = b.order_id');
    
    $result = $t->db->from("orders as a")->get()->row_array();
    //echo $t->db->last_query();
    
    return $result['total'];
}

function mfg_total($status = '')
{
    $t = &get_instance();
    $t->db->select('count(a.`id`) total');
    
    $t->db->join('users_groups ug','ug.user_id = a.id');
    $t->db->join('seller_data sd','sd.user_id = a.id');
    $where['ug.group_id'] = '4';
    $t->db->where($where);
    $t->db->where('a.active', 1);
    
    if($status!='')
    {
        $t->db->where('sd.status', $status);
    }
    
    $result = $t->db->from("users a")->get()->row_array();
    //echo $t->db->last_query();
    
    return $result['total'];
}

function retailer_total($status = '')
{
    $t = &get_instance();
    $t->db->select('count(a.`id`) total');
    
    $t->db->join('users_groups ug','ug.user_id = a.id');
    $t->db->join('retailer_data rd','rd.user_id = a.id');
    $where['ug.group_id'] = '2';
    $t->db->where($where);
    $t->db->where('a.active', 1);
...
function total_products_by_super_category($super_category_id, $status = '')
{
    $t = &get_instance();
    $t->db->select(' COUNT( distinct(p.id)) as `total` ')->join(" categories c", "p.category_id=c.id ")->join('product_variants', 'product_variants.product_id = p.id');
    
    if(isset($status) && $status != "") {
        if(is_array($status))
        {
            $t->db->where_in('p.status', $status);
        }
        else
        {
            $t->db->where("p.status", $status);
        }
    }
    
    if($super_category_id)
    {
        $t->db->where('p.super_category_id', $super_category_id);
    }
    
    $result = $t->db->get('products p')->row_array();
    //echo $t->db->last_query();
...
function mfg_orders_total($seller_id = "")
{
    $t = &get_instance();
    $t->db->select('sum(a.`final_total`) total');
    ...
    $t->db->join('order_items as b','a.id = b.order_id');
    if(!empty($seller_id)) {
        $t->db->where('b.seller_id', $seller_id);
    }
...
function mfg_orders_count($status = "", $seller_id = "")
{
    $t = &get_instance();
    $t->db->select('count(DISTINCT a.`id`) total');
    $t->db->join('order_items as b','a.id = b.order_id');
    if(!empty($status)) {
        if(is_array($status))
        {
            $t->db->where_in('a.order_status', $status);
        }
        else
        {
            $t->db->where('a.order_status', $status);
        }
    }
...
function admin_orders_count($status = "")
{
    $t = &get_instance();
    $t->db->select('count(DISTINCT `id`) total');
    if(!empty($status)) {
...
function get_category_parent_0($category_id)
{
    $t = &get_instance();
    
    $t->db->select('id, parent_id');
    $t->db->from('categories');
    $t->db->where('id', $category_id);
    $t->db->where('status', 1);
    $query = $t->db->get();
    ...
    {
        return get_category_parent_0($row['parent_id']);
    }
}