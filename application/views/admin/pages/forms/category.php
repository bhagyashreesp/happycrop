                    ...
                    <div class="card card-info">
                        ...
                        <form class="form-horizontal form-submit-event" action="<?= base_url('admin/category/add_category'); ?>" method="POST" id="add_product_form" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    <label for="super_category_id" class="col-sm-2 col-form-label">Select Super Category</label>
                                    <div class="col-sm-10">
                                        <select id="super_category_id" name="super_category_id" class="form-control">
                                            <option value="">Select</option>
                                            <?php
                                            $super_categories = $this->db->get_where('super_categories')->result_array();
                                            if($super_categories)
                                            {
                                                foreach($super_categories as $super_category)
                                                {
                                                    $selected = '';
                                                    if($super_category['id'] == $fetched_data[0]['super_category_id'])
                                                    {
                                                        $selected = 'selected="selected"';
                                                    }
                                                    ?>
                                                    <option value="<?php echo $super_category['id']; ?>" <?php echo $selected; ?>><?php echo $super_category['name']; ?></option>
                                                    <?php
                                                }
                                            }
                                            ?>
