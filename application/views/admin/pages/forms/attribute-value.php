                    ...
                    <div class="card card-info">
                        ...
                        <form class="form-horizontal form-submit-event" action="<?= base_url('admin/attribute_value/add_attribute_value'); ?>" method="POST" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    <label for="volume" class="col-sm-2 col-form-label">Volume / Weight <span class='text-danger text-sm'>*</span></label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="volume" placeholder="volume" name="volume" value="<?= @$fetched_data[0]['volume'] ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="unit_id" class="col-sm-2 col-form-label">Select Unit <span class='text-danger text-sm'>*</span></label>
                                    <div class="col-sm-10">
                                        <select class="form-control" id="unit_id" name="unit_id">
                                            <option value=""> None </option>
                                            <?php foreach ($units as $row) {
                                            ?>
                                                <option value="<?= $row['id'] ?>" <?= (isset($fetched_data[0]['unit_id']) && $fetched_data[0]['unit_id'] == $row['id']) ? 'selected' : '' ?>> <?= $row['unit'] ?>
                                                </option>
                                            <?php
                                            } ?>
                                        </select>
