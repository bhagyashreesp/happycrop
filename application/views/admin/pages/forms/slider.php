                        ...
                        <form class="form-horizontal form-submit-event" action="<?= base_url('admin/slider/add_slider'); ?>" method="POST" id="payment_setting_form" enctype="multipart/form-data">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <?php */ ?>
                                    <input type="hidden" name="slider_type" id="slider_type" value="default" />
                                </div>
                                <div class="form-group">
                                    <label>Show On</label><br />
                                    <?php
                                    $types = $this->db->get_where('slider_types', array('status'=>1))->result_array();
                                    if($types)
                                    {
                                        $slider_type_ids = array();
                                        
                                        if($fetched_data[0]['slider_type_ids'])
                                        {
                                            $slider_type_ids = @$fetched_data[0]['slider_type_ids'];
                                            $slider_type_ids = explode(',', $slider_type_ids);
                                        }
                                        ...
                                        foreach($types as $type)
                                        {
                                            ...
                                            <div class="checkbox d-inline">
                                                <label class="mr-3">
                                                    <input <?php echo (in_array($type['id'], $slider_type_ids)) ? 'checked="true"' : ''; ?> type="checkbox" name="slider_type_ids[]" value="<?php echo $type['id']; ?>" /> <?php echo $type['slider_type']; ?>
                                                </label>
                                            </div>
                                            <?php
                                        }
