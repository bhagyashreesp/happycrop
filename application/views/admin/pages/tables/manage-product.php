                        ...
                        if($super_categories)
                        {
                            foreach($super_categories as $row)
                            {
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-12 mb-3 text-center">
                                    ...
                                    <a href="<?php echo base_url().'admin/product/products/'.$row['id'].'/'.$product_status; ?>"><img class="p-2 wd-100" src="<?php echo base_url().$row['image'];?>" alt="<?php echo $row['name']; ?>" /></a>
                                    <h6 class="mt-2"><a href="<?php echo base_url().'admin/product/products/'.$row['id'].'/'.$product_status; ?>"><?php echo $row['name'];?></a></h6>
                                    </div>
                                </div>
                                <?php
                            }
                    ...
                    <div class="card content-area p-4">
                        ...
                        <div class="card-header border-0">
                            <div class="card-tools">
                                <a href="<?= base_url() . 'admin/product/new_product?super_category_id='.$super_category_id ?>" class="btn btn-block btn-outline-primary btn-sm">Add Product</a>
                            </div>
                        </div>
                        <div class="card-innr">
                            <div class="row">
                                ...
                                <?php } ?> 
                                <div class="col-md-3">
                                    <label for="zipcode" class="col-form-label">Filter By Manufacturer</label>
                                    <select class='form-control' name='seller_id' id="seller_filter">
                                        <option value="">Select Manufacturer </option>
                                        <?php foreach ($sellers as $seller) { ?>
                                            <option value="<?= $seller['seller_id'] ?>" <?= (isset($product_details[0]['seller_id']) && $product_details[0]['seller_id'] == $seller['seller_id']) ? 'selected' : "" ?>><?= $seller['company_name'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                            <div class="gaps-1-5x"></div>
                            <table class='table-striped' id='products_table' data-toggle="table" data-url="<?= isset($_GET['flag']) ? base_url('admin/product/get_product_data/'.$super_category_id.'/'.$product_status.'/?flag=') . $_GET['flag'] : base_url('admin/product/get_product_data/'.$super_category_id.'/'.$product_status) ?>" data-click-to-select="true" data-side-pagination="server" data-pagination="true" data-page-list="[5, 10, 20, 50, 100, 200]" data-search="true" data-show-columns="true" data-show-refresh="true" data-trim-on-search="false" data-sort-name="id" data-sort-order="desc" data-mobile-responsive="true" data-toolbar="" data-show-export="true" data-maintain-selected="true" data-export-types='["txt","excel","csv"]' data-export-options='{
                            ...
                            "ignoreColumn": ["state"] 
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th data-field="activate" data-sortable="false">Activate / Deactivate</th>
                                        <th data-field="product_stock_status" data-sortable="false">In stock / Out of stock</th>
