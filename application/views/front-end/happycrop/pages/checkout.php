<style>.shipped-details p, .shipped-details2 p {margin-bottom: 5px;}</style>
<!-- breadcrumb -->
            ...
            <div class="row">
                <div class="col-md-8 bg-white mt-5">
                    ...
                    <div class="align-item-center ship-title-details justify-content-between user-add d-flex">
                        <h5 class="pb-0 mt-2 mb-2"><?= !empty($this->lang->line('billing_details')) ? $this->lang->line('billing_details') : 'Billing Address' ?></h5>
                    </div>
                    <div class="shipped-details2 mt-3 mb-3">
                        <p class="text-muted" id="address-name-type2"><?= isset($billing_address) && !empty($billing_address) ? $billing_address[0]['name'] . ' - ' . ucfirst($billing_address[0]['type']) : '' ?></p>
                        <p class="text-muted" id="address-full2"><?= isset($billing_address) && !empty($billing_address) ? $billing_address[0]['address'] . ' , ' . $billing_address[0]['city'] : '' ?></p>
                        <p class="text-muted" id="address-country2"><?= isset($billing_address) && !empty($billing_address) ? $billing_address[0]['state_name'] . ' , ' . $billing_address[0]['country_name'] . ' - ' . $billing_address[0]['pincode'] : '' ?></p>
                        <p class="text-muted" id="address-mobile2"><?= isset($billing_address) && !empty($billing_address) ? $billing_address[0]['mobile'] : '' ?></p>
                    </div>
                    <div class="ship-details-wrapper">
                        <div class="align-item-center ship-title-details justify-content-between user-add d-flex">
                            <h5 class="pb-0 mt-2 mb-0"><?= !empty($this->lang->line('shipping_address')) ? $this->lang->line('shipping_address') : 'Shipping Address' ?></h5>
                            <a class="button button-primary mt-2 mb-1 text-white" href="#" data-izimodal-open=".address-modal"><i class="fas fa-edit edit-icon text-white"></i> Change Address</a>
                        </div>
                        <div class="shipped-details mt-3">
                            ...
                            <p class="text-muted" id="address-full"><?= isset($default_address) && !empty($default_address) ? $default_address[0]['address'] . ' , ' . $default_address[0]['city'] : '' ?></p>
                            <p class="text-muted" id="address-country"><?= isset($default_address) && !empty($default_address) ? $default_address[0]['state_name'] . ' , ' . $default_address[0]['country_name'] . ' - ' . $default_address[0]['pincode'] : '' ?></p>
                        ...
                        <input type="hidden" name="billing_address_id" id="billing_address_id" value="<?= isset($billing_address) && !empty($billing_address) ? $billing_address[0]['id'] : '' ?>" />
                        ...
                        <input type="hidden" name="is_service_category" value="<?php echo $cart[0]['is_service_category']; ?>" />
                    </div>
                    ...
                    <div class="align-item-center hidden ship-title-details justify-content-between d-flex">
                    ...
                    <div class="form-check d-flex hidden">
                    ...
                    <div class="ship-details-wrapper mt-3 payment-methods d-none"  style="opacity: 0;">
                                    ...
                                    <?php if (isset($payment_methods['cod_method']) && $payment_methods['cod_method'] == 1) { ?>
                                        <tr>
                                            <label for="cod">
                                                <td>
                                                    <label for="cod">
                                                        <input checked="" id="cod" title="<?= isset($cart[0]['is_cod_allowed']) && $cart[0]['is_cod_allowed'] == 0 ? 'Cash on delivery is not allowed for one of the item in your cart' : 'Please select one of this options.' ?>" name="payment_method" type="radio" value="COD" <?= isset($cart[0]['is_cod_allowed']) && $cart[0]['is_cod_allowed'] == 0 ? 'disabled' : '' ?>>
                                                    </label>
                                                </td>
                                                <td>
                                                    <label for="cod">
                ...
                <div class="col-md-4 mt-5">
                    <div class="checkout-order-wrapper p-3">
                            ...
                            <div class="product-checkout-wrapper">
                                ...
                                <div class="cart-total-price">
                                    <table class="table cart-products-table">
                                        ...
                                        <tfoot>
                                            <?php 
                                            $total_mrp = !empty($cart['total_mrp']) ? number_format($cart['total_mrp'], 2) : 0;
                                            $total_standard_price = !empty($cart['total_standard_price']) ? number_format($cart['total_standard_price'], 2) : 0;
                                            
                                            $total = !empty($cart['sub_total']) ? number_format($cart['overall_amount'] - $cart['delivery_charge'], 2) : 0;
                                            
                                            $profit_gain = $cart['total_standard_price'] - $cart['sub_total'];
                                            $profit_gain = !empty($profit_gain) ? number_format($profit_gain, 2) : 0;
                                            
                                            $profit_gain_percent = ($cart['total_standard_price'] - $cart['sub_total']) * 100/$cart['total_standard_price'];
                                            $profit_gain_percent = !empty($profit_gain_percent) ? number_format($profit_gain_percent, 2).'%' : 0;
                                            
                                            ?>
                                            <tr class="total-mrp-price">
                                                <td>Standard Total price<br /> (inc GST)</td>
                                                <td class="t_mrp"><?= $settings['currency'] . '' . $total_standard_price ?></td>
                                            </tr>
                                ...
                                <div class="input-group2">
                                    <input type="hidden" class="form-control" placeholder="Promo code" id="promocode_input">
                                    <?php /* ?>
                                ...
                                <button class="block" id="place_order_btn" disabled="true" type="submit" <?= ($is_disabled) ? "disabled" : ""; ?>><?= !empty($this->lang->line('place_order')) ? $this->lang->line('place_order') : 'Place Order' ?></button>
...
<div id="modal-custom" class="address-modal" data-iziModal-group="grupo1">
    ...
    <section id="address_form">
        ...
        <div class="col-12 text-right mt-2">
            <a target="_self" href="<?= base_url('my-account/business-details/0') ?>">Manage Sales & Storage Address</a>
        </div>
        <footer class="mt-4">
