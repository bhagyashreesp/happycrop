...
class Ion_auth
{
    ...
    public function forgotten_password2($identity, $is_seller)
    {
		...
		if($user)
		{
			...
			if ($code)
			{
                ...
                $overall_order_data = array(
                    ...
                    'user_msg'   => 'Hello '.$user->username.', For reset password click on the below link <br/> <a href="'.base_url('register/set-password/'.$code.'/'.$is_seller).'" target="_blank">Reset Password</a>',
                    //'otp_msg'    => 'Here is your OTP. Please, give it to delivery boy only while getting your order.',
                );
                
                send_mail2($user->email, 'Forget Password', $this->load->view('admin/pages/view/order-email-template.php', $overall_order_data, TRUE));
    ...
    public function member_status($id = FALSE)
	{
        $id = ($id == FALSE) ? $this->session->userdata('user_id') : $id;
        if(!empty($id) && $id != FALSE){
            $status = fetch_details(['user_id' => $id], 'retailer_data','status');
            return $status[0]['status'];
        }else{
            return false;
        }
		...
		return $this->ion_auth_model->in_group($default_group, $id);
	}
