...
class Seller_model extends CI_Model
{
    ...
    function add_custom_seller($data, $profile = [], $com_data = [])
    {
        $data = escape_array($data);
        $profile = (!empty($profile)) ? escape_array($profile) : [];
        $com_data = (!empty($com_data)) ? escape_array($com_data) : [];
        ...
        if (isset($data['edit_seller_data_id'])) {
            if (!empty($com_data)) {
                ...
                for ($i = 0; $i < count($com_data); $i++) {
                    $where = "";
                    if (is_exist(['category_id' => $com_data[$i]['category_id'], 'seller_id' => $com_data[$i]['seller_id']], 'seller_commission')) {
                        $update_com = ['commission' => $com_data[$i]['commission']];
                        $where = "category_id = " . $com_data[$i]['category_id'] . " and seller_id=" . $com_data[$i]['seller_id'];
                        $this->db->set($update_com)->where($where)->update('seller_commission');
                    } else {
                        $this->db->insert('seller_commission', $com_data[$i]);
                    }
                }
            }
    ...
    function get_sellers_list($status = '')
    {
        $offset = 0;
        $limit = 10;
        $sort = 'u.id';
        ...
        $count_res = $this->db->select(' COUNT(u.id) as `total` ')->join('users_groups ug', ' ug.user_id = u.id ')->join('seller_data sd', ' sd.user_id = u.id ')->join('cities ct', ' sd.city_id = ct.id ','left');
        ...
        $search_res = $this->db->select(' u.*,sd.*, ct.name as city_name ')->join('users_groups ug', ' ug.user_id = u.id ')->join('seller_data sd', ' sd.user_id = u.id ')->join('cities ct', ' sd.city_id = ct.id ','left');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
        ...
        foreach ($offer_search_res as $row) {
            ...
            //$operate = " <a href='manage-new-seller?edit_id=" . $row['user_id'] . "' data-id=" . $row['user_id'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
            $operate = " <a href='".base_url()."admin/sellers/edit-seller?edit_id=" . $row['user_id'] ."' data-id=" . $row['user_id'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
            ...
            if ($row['status'] == '1' || $row['status'] == '0' || $row['status'] == '2') {
                $operate .= '<a  href="javascript:void(0)" class="remove-sellers btn btn-warning btn-xs mr-1 mb-1" title="Remove Manufacturer"  data-id="' . $row['user_id'] . '" data-seller_status="' . $row['status'] . '" ><i class="fas fa-user-slash"></i></a>';
            } else if ($row['status'] == '7') {
                $operate .= '<a  href="javascript:void(0)" class="remove-sellers btn btn-primary btn-xs mr-1 mb-1" title="Restore Manufacturer"  data-id="' . $row['user_id'] . '" data-seller_status="' . $row['status'] . '" ><i class="fas fa-user"></i></a>';
            }
            $tempRow['id'] = $row['id'];
            $tempRow['mfg_no'] = 'HCM'.str_pad($row['mfg_no'],4,"0", STR_PAD_LEFT);
            $tempRow['name'] = $row['username'];
            $tempRow['email'] = $row['email'];
            $tempRow['mobile'] = $row['mobile'];
            $tempRow['address'] = $row['address'];
            $tempRow['company_name'] = '<a href="'.base_url('products?seller='.$row['slug']).'">'.$row['company_name'].'</a>';
    ...
    function settle_seller_commission()
    {
        $date = date('Y-m-d');
        ...
        foreach ($data as $row) {
            $cat_com = fetch_details(['seller_id' => $row['seller_id'], 'category_id' => $row['category_id']], 'seller_commission', 'commission');
            if (!empty($cat_com)) {
                $commission_pr = $cat_com[0]['commission'];
            } else {
                $global_comm = fetch_details(['user_id' => $row['seller_id']], 'seller_data', 'commission');
