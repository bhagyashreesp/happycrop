...
class Retailer_model extends CI_Model
{
    ...
    function get_retailers_list()
    {
        ...
        $count_res = $this->db->select(' COUNT(u.id) as `total` ')->join('users_groups ug', ' ug.user_id = u.id ')->join('retailer_data sd', ' sd.user_id = u.id ');
        ...
        $search_res = $this->db->select(' u.*,sd.* ')->join('users_groups ug', ' ug.user_id = u.id ')->join('retailer_data sd', ' sd.user_id = u.id ');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
        ...
        foreach ($offer_search_res as $row) {
            ...
            $operate = " <a href='edit-retailer?edit_id=" . $row['user_id'] . "' data-id=" . $row['user_id'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
