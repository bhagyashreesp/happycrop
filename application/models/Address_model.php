...
class Address_model extends CI_Model
{
    ...
    function get_address($user_id, $id = false, $fetch_latest = false, $is_default = false)
    {
        ...
        if (isset($user_id) || $id != false) {
            ...
            $this->db->select('addr.*,a.name as area,a.minimum_free_delivery_order_amount,a.delivery_charges,c.name as city, s.name as state_name, cs.name as country_name')
                ->where($where)
                ->join('cities c', 'addr.city_id=c.id', 'inner')
                ->join('areas a', 'addr.area_id=a.id', 'left')
                ->join('states s', 's.id=c.state_id', 'left')
                ->join('countries cs', 'cs.id=s.country_id', 'left')
    ...
    public function get_billing_address($user_id, $id = false, $fetch_latest = false, $is_default = false)
    {
        $where = [];
        if (isset($user_id) || $id != false) {
            if (isset($user_id) && $user_id != null && !empty($user_id)) {
    ...
    public function get_address_list($user_id = '', $type = '')
    {
        $offset = 0;
        $limit = 10;
        $sort = 'id';
        ...
        $count_res = $this->db->select(' COUNT(addr.id) as `total` ,addr.*,a.name as area,c.name as city, s.name as state_name')->join('cities c', 'addr.city_id=c.id', 'left')->join('areas a', 'addr.area_id=a.id', 'left')->join('states s', 'addr.state_id=s.id', 'left');
        ...
        if($type!='')
        {
            $count_res->where('addr.type', $type);
        }
        ...
        $search_res = $this->db->select('addr.*,a.name as area,c.name as city, s.name as state_name')
                        ->join('cities c', 'addr.city_id=c.id', 'left')
                        ->join('areas a', 'addr.area_id=a.id', 'left')
                        ->join('states s', 'addr.state_id=s.id', 'left');
        ...
        if($type!='')
        {
            $search_res->where('addr.type', $type);
        }
        ...
        foreach ($address_search_res as $row) {
            ...
            //$operate = '<a href="javascript:void(0)" class="edit-address btn btn-success btn-xs btn-sm mr-1 mb-1" title="Edit" data-id="' . $row['id'] . '" data-toggle="modal" data-target="#address-modal"><i class="fa fa-pen"></i></a>';
            $operate = '<a href="javascript:void(0)" onclick="AjaxModal(\''.base_url('my-account/modal_add_address/'.$row['id']).'\',\'Edit Address\')" class="btn btn-success bg-success btn-xs btn-sm mr-1 mb-1" title="Edit"><i class="fa fa-pen"></i></a>';
            ...
            //$operate .= '<a href="javascript:void(0)" class="'.$class.' btn btn-'.$btn.' btn-xs btn-sm mr-1 mb-1" title="' . $default . '" data-id="' . $row['id'] . '"><i class="fa fa-check-square"></i></a>';
            if($row['add_order'] == 3)
            {
                $operate .= '<a href="javascript:void(0)" class="delete-address btn btn-danger btn-xs btn-sm mr-1 mb-1" title="Delete" data-id="' . $row['id'] . '"><i class="fa fa-trash"></i></a>';
            }
            
            $tempRow['id'] = $row['id'];
            $tempRow['shop_name'] = $row['shop_name'];
            $tempRow['plot_no'] = $row['plot_no'];
            $tempRow['street_locality'] = $row['street_locality'];
            $tempRow['state_name'] = $row['state_name'];
            $tempRow['district_name'] = $row['city'];
