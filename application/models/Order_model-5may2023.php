...
class Order_model extends CI_Model
{
    ...
    public function get_order_list($user_id = '', $status = array())
    {
        ...
        $search_res = $this->db->select(' o.* , u.username, sd.company_name')//, db.username as delivery_boy
            ->join(' `users` u', 'u.id= o.user_id', 'left')
            ->join(' `order_items` oi', 'oi.order_id= o.id', 'left')
            ->join('users us ', ' us.id = oi.seller_id', 'left')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
    ...
    {
        $res = $this->db->select('oi.*, sd.company_name as seller_name, u.username, r.company_name, r.gst_no as r_gst_no, r.fertilizer_license_no, r.pesticide_license_no, r.seeds_license_no, v.mrp,ot.courier_agency,ot.tracking_id,ot.url,oi.otp as item_otp,a.name as user_name,oi.id as order_item_id,p.*,v.product_id,o.*,o.id as order_id,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.username as uname,oi.status as order_status,p.name as pname,p.type,p.image as product_image,p.is_prices_inclusive_tax,(SELECT username FROM users db where db.id=oi.delivery_boy_id ) as delivery_boy, v.packing_size, uu.unit, v.carton_qty, tax.percentage as tax_percentage ')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
            ->join('products p ', ' p.id = v.product_id ', 'left')
            ->join('users u ', ' u.id = oi.user_id', 'left')
    ...
    public function get_seller_orders_list(
        ...
        $search_res = $this->db->select(' o.* , u.username, rd.company_name as retailer_name')//, db.username as delivery_boy
            ->join(' `users` u', 'u.id= o.user_id', 'left')
            ->join(' `order_items` oi', 'oi.order_id= o.id', 'left')
            ->join('users us ', ' us.id = oi.seller_id', 'left')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
    ...
    public function get_orders_list(
        ...
        $search_res = $this->db->select(' o.* , u.username, rd.company_name as retailer_name, sd.company_name as mfg_name')
            ->join(' `users` u', 'u.id= o.user_id', 'left')
            ->join(' `order_items` oi', 'oi.order_id= o.id', 'left')
            ->join('users us ', ' us.id = oi.seller_id', 'left')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
