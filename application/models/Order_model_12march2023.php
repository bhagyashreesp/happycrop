...
class Order_model extends CI_Model
{
        ...
        foreach ($user_details as $row) {
            if (!empty($row['items'])) {
                ...
                //if (!$this->ion_auth->is_delivery_boy()) {
                    ...
                    $operate .= '<a onclick="re_order_the_order(' . $row['id'] . ')" href="javascript:void(0);" class="btn btn-secondary btn-sm mr-1 mb-1" title="View" >Re-Order</a>';
    ...
    {
        $res = $this->db->select('oi.*, u.username, r.company_name, r.gst_no as r_gst_no, r.fertilizer_license_no, r.pesticide_license_no, r.seeds_license_no, v.mrp,ot.courier_agency,ot.tracking_id,ot.url,oi.otp as item_otp,a.name as user_name,oi.id as order_item_id,p.*,v.product_id,o.*,o.id as order_id,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.username as uname,oi.status as order_status,p.name as pname,p.type,p.image as product_image,p.is_prices_inclusive_tax,(SELECT username FROM users db where db.id=oi.delivery_boy_id ) as delivery_boy, v.packing_size, uu.unit, v.carton_qty, tax.percentage as tax_percentage ')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
            ->join('products p ', ' p.id = v.product_id ', 'left')
            ->join('users u ', ' u.id = oi.user_id', 'left')
    ...
    public function add_payment_confirmation($data)
    {
        ...
        $this->db->update('orders', array('order_status' => 'send_payment_confirmation'), array('id' => $data['order_id']));
    ...
    public function add_invoice($data)
    {
        ...
        $this->db->update('orders', array('order_status' => 'send_invoice'), array('id' => $data['order_id']));
