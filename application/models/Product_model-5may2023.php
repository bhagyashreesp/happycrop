...
class Product_model extends CI_Model
{
    ...
    public function get_product_details($flag = NULL, $seller_id = NULL,$p_status = NULL,$super_category_id = 0)
    {
        ...
        $search_res = $this->db->select('product_variants.id AS id, p.id as pid ,p.rating,p.no_of_ratings,p.name, p.type, p.image, p.status,product_variants.price , product_variants.special_price, product_variants.stock, p.super_category_id ')
        ...
        foreach ($pro_search_res as $row) {
            ...
            {
                $operate .= " <a href='".base_url()."admin/product/new_product/new-product?edit_id=" . $row['pid'] . "&super_category_id=" . $row['super_category_id'] . "' data-id=" . $row['pid'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
            }
            else
            {
                $operate .= " <a href='".base_url()."seller/product/new_product/new-product?edit_id=" . $row['pid'] . "&super_category_id=" . $row['super_category_id'] . "' data-id=" . $row['pid'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
            }
            if ($row['status'] == '2') {
