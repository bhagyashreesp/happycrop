...
class Attribute_model extends CI_Model
{
    ...
    public function get_attribute_values(
        ...
        $count_res = $this->db->select(' COUNT(attr_vals.id) as `total` ')
            ...
            ->join('units un', 'attr_vals.unit_id=un.id', 'left');
        ...
        $search_res = $this->db->select(' attr_vals.*,attr.name as attr_name, un.unit')->join('attributes attr', 'attr.id=attr_vals.attribute_id', 'left')->join('units un', 'attr_vals.unit_id=un.id', 'left');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
            $search_res->or_like($multipleWhere);
        }
        if (isset($where) && !empty($where)) {
    ...
    function get_attribute_set($sort = "name", $order = "ASC", $search = "")
    {
        $multipleWhere = '';
        $where = array();
        if (!empty($search)) {
        ...
        $attribute_set = $search_res->where("status=1")->order_by($sort, $order)->get('attribute_set')->result_array();
        $bulkData = array();
    ...
    function get_attributes($sort = "a.name", $order = "ASC", $search = "", $attribute_set_id = "")
    {
        $multipleWhere = '';
        $where = array();
        if (!empty($search)) {
        ...
        $search_res = $this->db->select('a.*,as.name as attribute_set_name')->join('attribute_set as', 'as.id=a.attribute_set_id');
        if (isset($multipleWhere) && !empty($multipleWhere)) {
            $search_res->group_start();
            $search_res->or_like($multipleWhere);
            $search_res->group_end();
        ...
        $attribute_set = $search_res->where("a.status=1")->order_by($sort, $order)->get('attributes a')->result_array();
        $bulkData = array();
        $bulkData['error'] = (empty($attribute_set)) ? true : false;
        if (!empty($attribute_set)) {
            for ($i = 0; $i < count($attribute_set); $i++) {
    ...
    function get_attribute_value($sort = "av.id", $order = "ASC", $search = "", $attribute_id = "")
    {
        $multipleWhere = '';
        $where = array();
        if (!empty($search)) {
        ...
        $attribute_set = $search_res->where("av.status=1")->order_by($sort, $order)->get('attribute_values av')->result_array();
        $bulkData = array();
        $bulkData['error'] = (empty($attribute_set)) ? true : false;
        if (!empty($attribute_set)) {
            for ($i = 0; $i < count($attribute_set); $i++) {
