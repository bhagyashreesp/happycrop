...
class Product_model extends CI_Model
{
    ...
    public function add_product($data)
    {
        ...
        $technician     = $data['pro_input_technician'];
        $tools          = $data['pro_input_tools'];
        $certifications = $data['pro_input_certifications'];
        ...
        $min_area_req = $data['min_area_req'];
        ...
        if (isset($data['edit_product_id'])) {
            ...
            $this->db->select('a.id, a.serv_no, a.pro_no, b.is_service_category');
            $this->db->from('products as a');
            $this->db->join('super_categories as b','a.super_category_id = b.id');
            $this->db->where('a.id', $data['edit_product_id']);
            $query  = $this->db->get();
            
            $row    = $query->row_array();
            if($row['is_service_category'] && !$row['serv_no'])
            {
            ...
            else if(!$row['is_service_category'] && !$row['pro_no'])
            {
        ...
        $p_id = (isset($data['edit_product_id'])) ? $data['edit_product_id'] : $p_id;
        ...
        if ($pro_type == 'simple_product') {
            ...
            if (isset($data['variant_stock_status']) && $data['variant_stock_status'] == '0') {
                ...
                if ($data['variant_stock_level_type'] == "product_level") {
                    ...
                    $variant_sell_price_per_item = $data['variant_sell_price_per_item'];
            ...
            if (!empty($data['variants_ids'])) {
                ...
                for ($i = 0; $i < count($variants_ids); $i++) {
                    ...
                    if ($flag == "variant_level") {
                        ...
                        $car_qty   = ($variant_carton_qty[$i]) ? $variant_carton_qty[$i] : 1;
                        if(round($variant_sell_price_per_item[$i]) > 0)
                        {
                            $spl_price = ($variant_special_price_per_item[$i] + ($variant_sell_price_per_item[$i] - $variant_special_price_per_item[$i])*10/100) * $car_qty; 
                            $pro_variance_data['special_price'] = $spl_price;// (isset($variant_special_price[$i]) && !empty($variant_special_price[$i])) ? $variant_special_price[$i] : '0';
                            
                            $pro_variance_data['mrp'] = $variant_sell_price_per_item[$i] * $car_qty;
                        }
                        else
                        {
                            $spl_price = ($variant_special_price_per_item[$i] + ($variant_mrp_per_item[$i] - $variant_special_price_per_item[$i])*10/100) * $car_qty; 
                            $pro_variance_data['special_price'] = $spl_price;// (isset($variant_special_price[$i]) && !empty($variant_special_price[$i])) ? $variant_special_price[$i] : '0';
                        ...
                        $pro_variance_data['sell_price_per_item'] = $variant_sell_price_per_item[$i];
            ...
            if ($pro_type == 'carton_product') {
                ...
                for ($i = 0; $i < count($variant_mrp); $i++) {
                    ...
                    if ($flag == "variant_level") {
                        ...
                        {
                            $spl_price = ($variant_special_price_per_item[$i] + ($variant_sell_price_per_item[$i] - $variant_special_price_per_item[$i])*10/100) * $car_qty;
                            $pro_variance_data['mrp'] = ($variant_carton_qty[$i]) ? $variant_carton_qty[$i]*$variant_sell_price_per_item[$i] : 1*$variant_sell_price_per_item[$i];//$variant_mrp[$i];
                        }
                        else
                        {
                            $spl_price = ($variant_special_price_per_item[$i] + ($variant_mrp_per_item[$i] - $variant_special_price_per_item[$i])*10/100) * $car_qty;
                        ...
                        $pro_variance_data['special_price'] = $spl_price;// ($variant_carton_qty[$i]) ? $variant_carton_qty[$i]*$variant_special_price_per_item[$i] : 1*$variant_special_price_per_item[$i];//(isset($variant_special_price[$i]) && !empty($variant_special_price[$i])) ? $variant_special_price[$i] : '0';
    ...
    public function get_product_details($flag = NULL, $seller_id = NULL,$p_status = NULL,$super_category_id = 0)
    {
        ...
        if (isset($_GET['search_filter']) and $_GET['search_filter'] != '') {
            $search_filter = trim($_GET['search_filter']);
            $multipleWhere = ['p.`id`' => $search_filter, 'p.`name`' => $search_filter, 'p.`description`' => $search_filter, 'p.`short_description`' => $search_filter, 'c.name' => $search_filter];
        }
        ...
        $count_res = $this->db->select(' COUNT( distinct(p.id)) as `total` ')->join(" categories c", "p.category_id=c.id ")->join('product_variants', 'product_variants.product_id = p.id')->join('super_categories as sc','p.super_category_id = sc.id');
        ...
        $search_res = $this->db->select('product_variants.id AS id, p.id as pid ,p.rating,p.no_of_ratings,p.name, p.slug, p.type, p.image, p.status, p.product_stock_status, product_variants.price , product_variants.special_price, product_variants.stock, p.super_category_id, sc.is_service_category, p.pro_no, p.serv_no ')
            ->join(" categories c", "p.category_id=c.id ")->join('product_variants', 'product_variants.product_id = p.id')->join('super_categories as sc','p.super_category_id = sc.id');
        ...
        foreach ($pro_search_res as $row) {
            ...
            if ($row['status'] == '2') {
                $tempRow['status'] = '<a class="badge badge-danger text-white" >Approval Pending</a>';
                if ($this->ion_auth->is_seller()) {
                    $activate .= '<a class="btn btn-secondary mr-1 mb-1 btn-sm" data-table="products" href="javascript:void(0)" title="Approval Pending" >Approval Pending</a>';
                }else{
                    $activate .= '<a class="btn btn-success mr-1 mb-1 btn-sm update_active_status" data-table="products" href="javascript:void(0)" title="Approve" data-id="' . $row['pid'] . '" data-status="' . $row['status'] . '" ><i class="fa fa-check"></i> Approve</a>';
                    $activate .= '<a class="btn btn-danger mr-1 mb-1 btn-sm update_active_status" data-table="products" href="javascript:void(0)" title="Reject" data-id="' . $row['pid'] . '" data-status="3" ><i class="fa fa-ban"></i> Reject</a>';
                }
            }
            ...
            $tempRow['id'] = $row['pid'];
            $tempRow['no'] = ($row['is_service_category']) ? 'HCS'.str_pad($row['serv_no'],6,"0",STR_PAD_LEFT) : 'HCP'.str_pad($row['pro_no'],6,"0",STR_PAD_LEFT);
