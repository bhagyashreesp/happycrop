...
class Setting_model extends CI_Model
{
    ...
    public function update_quality_policy($post)
    {
        $post = escape_array($post);

        $query = $this->db->get_where('settings', array(
            'variable' => 'quality_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
                'variable' => 'quality_policy',
                'value' => $post['quality_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['quality_policy_input_description'])->where('variable', 'quality_policy')->update('settings');
        }
    }
    
    public function update_pricing_policy($post)
    {
        $post = escape_array($post);

        $query = $this->db->get_where('settings', array(
            'variable' => 'pricing_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
                'variable' => 'pricing_policy',
                'value' => $post['pricing_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['pricing_policy_input_description'])->where('variable', 'pricing_policy')->update('settings');
        }
    }
    
    public function update_delivery_policy($post)
    {
        $post = escape_array($post);
        ...
        if ($count === 0) {
            $data = array(
                'variable' => 'delivery_policy',
                'value' => $post['delivery_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['delivery_policy_input_description'])->where('variable', 'delivery_policy')->update('settings');
        }
    }
    
    public function update_payment_policy($post)
    {
        $post = escape_array($post);

        $query = $this->db->get_where('settings', array(
            'variable' => 'payment_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
                'variable' => 'payment_policy',
                'value' => $post['payment_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['payment_policy_input_description'])->where('variable', 'payment_policy')->update('settings');
        }
    }
    
    public function update_security_policy($post)
    {
        $post = escape_array($post);
        ...
        if ($count === 0) {
            $data = array(
                'variable' => 'security_policy',
                'value' => $post['security_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['security_policy_input_description'])->where('variable', 'security_policy')->update('settings');
        }
    }
    
    public function update_partnering_policy($post)
    {
        $post = escape_array($post);

        $query = $this->db->get_where('settings', array(
            'variable' => 'partnering_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
                'variable' => 'partnering_policy',
                'value' => $post['partnering_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['partnering_policy_input_description'])->where('variable', 'partnering_policy')->update('settings');
        }
    }
    
    public function update_licensing_policy($post)
    {
        $post = escape_array($post);

        $query = $this->db->get_where('settings', array(
            'variable' => 'licensing_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
                'variable' => 'licensing_policy',
                'value' => $post['licensing_policy_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['licensing_policy_input_description'])->where('variable', 'licensing_policy')->update('settings');
        }
    }
    ...
    public function update_careers($post)
    {
        $query = $this->db->get_where('settings', array(
            'variable' => 'careers'
        ));
        ...
        if ($count === 0) {
            $data = array(
                'variable' => 'careers',
                'value' => $post['careers_input_description']
            );
            $this->db->insert('settings', $data);
        } else {
            $this->db->set('value', $post['careers_input_description'])->where('variable', 'careers')->update('settings');
        }
    }
