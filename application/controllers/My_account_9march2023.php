...
class My_account extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->data['is_logged_in']) {
            ...
            if(!$is_seller)
            {
                ...
                $this->data['issue_raised_orders'] = retailer_orders_count("complaint", $this->data['user_id']);
                $this->data['cancelled_orders'] = retailer_orders_count("cancelled", $this->data['user_id']);
                $this->data['delivered_orders'] = retailer_orders_count("delivered", $this->data['user_id']);
            }
    ...
    public function save_step4()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            else
            {
                ...
                $retailer_data = ['company_name' => $this->input->post('company_name'), 'store_name' => $this->input->post('company_name'), 'slug' => create_unique_slug($this->input->post('company_name', true), 'retailer_data'), 'year_establish' => $this->input->post('year_establish'), 'owner_name' => $this->input->post('owner_name'), 'logo' => $logo_doc, 'plot_no' => $this->input->post('plot_no'), 'street_locality' => $this->input->post('street_locality'), 'landmark' => $this->input->post('landmark'), 'pin' => $this->input->post('pin'), 'city_id' => $this->input->post('city_id'), 'state_id' => $this->input->post('state_id'),'storage_plot_no' => $this->input->post('storage_plot_no'), 'storage_street_locality' => $this->input->post('storage_street_locality'), 'storage_landmark' => $this->input->post('storage_landmark'), 'storage_pin' => $this->input->post('storage_pin'), 'storage_city_id' => $this->input->post('storage_city_id'), 'storage_state_id' => $this->input->post('storage_state_id'),'website_url'=>$this->input->post('website_url'),'google_business_url'=>$this->input->post('google_business_url'),'facebook_business_url'=>$this->input->post('facebook_business_url'),'instagram_business_url'=>$this->input->post('instagram_business_url')];
                ...
                if($check2)
                {
                    $address_2_data = ['name'=>$user_info->username, 'mobile'=> $user_info->mobile, 'alternate_mobile'=> $user_info->alternate_mobile,'type'=>'storage','address'=>$this->input->post('storage_plot_no').' '.$this->input->post('storage_street_locality').' '.$this->input->post('storage_landmark'), 'landmark'=> $this->input->post('storage_landmark'), 'city_id' => $this->input->post('storage_city_id'), 'state_id' => $this->input->post('storage_state_id'), 'pincode' => $this->input->post('storage_pin'),'add_order'=>2];
                ...
                {
                    $address_2_data = ['name'=>$user_info->username, 'mobile'=> $user_info->mobile, 'alternate_mobile'=> $user_info->alternate_mobile,'user_id'=>$user_info->id,'type'=>'storage','address'=>$this->input->post('storage_plot_no').' '.$this->input->post('storage_street_locality').' '.$this->input->post('storage_landmark'), 'landmark'=> $this->input->post('storage_landmark'), 'city_id' => $this->input->post('storage_city_id'), 'state_id' => $this->input->post('storage_state_id'), 'pincode' => $this->input->post('storage_pin'),'add_order'=>2];
                    $address_2_data = escape_array($address_2_data);
    ...
    public function save_step5()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if($is_seller)
            {
                $seller_data = ['account_name'=>$this->input->post('account_name'),'account_number' => $this->input->post('account_number'), 'bank_code' => $this->input->post('bank_code'), 'bank_name' => $this->input->post('bank_name'), 'bank_branch' => $this->input->post('bank_branch'), 'bank_city' => $this->input->post('bank_city'), 'bank_state' => $this->input->post('bank_state')];
                $seller_data = escape_array($seller_data);
                $this->db->set($seller_data)->where('user_id', $this->input->post('user_id'))->update('seller_data');
    ...
    {
        if ($this->ion_auth->logged_in()) {
            return $this->order_model->get_order_list($this->data['user']->id);
        } else {
            $this->response['error'] = true;
            $this->response['message'] = 'Unauthorized access is not allowed';
            print_r(json_encode($this->response));
