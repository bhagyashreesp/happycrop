...
class Auth extends CI_Controller
{
    ...
    public function create_custom_user()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->ion_auth->register($identity, $password, $email, $additional_data, ['2']);
            if (update_details(['active' => 1], [$identity_column => $identity], 'users')) {
                ...
                $insert_id = $this->Seller_model->add_custom_retailer($data);
                ...
                if (!empty($insert_id)) {
                    ...
                    if($this->ion_auth->login($identity, $password, $remember)) {
                        ...
                        $this->response['redirect_to'] = base_url('register/step4/0');
                        $this->response['error'] = false;
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                        $this->response['message'] = 'Your are registared Successfully.';
                        print_r(json_encode($this->response));    
                    }
