...
class Cart extends CI_Controller
{
    ...
    public function re_order()
    {
        if ($this->data['is_logged_in']) {
            ...
            $this->db->select('product_variant_id, quantity');
            ...
            if($order_items)
            {
                ...
                foreach($order_items as $order_item)
                {
                    ...
                    $current_cart_items = get_cart_total($this->data['user']->id,  $order_item['product_variant_id']);
                    $current_qty = $current_cart_items['quantity'];
                    
                    $data = array(
                        'product_variant_id' => $order_item['product_variant_id'],
                        'qty' => $current_qty + $order_item['quantity'],
                        'is_saved_for_later' => false,
                        'user_id' => $this->data['user']->id,
                    );
                    $_POST['user_id'] = $this->data['user']->id;
                    $settings = get_settings('system_settings', true);
                    $cart_count = get_cart_count($this->data['user']->id);
                    $is_variant_available_in_cart = is_variant_available_in_cart($order_item['product_variant_id'], $this->data['user']->id);
                    ...
                    if (!$this->cart_model->add_to_cart($data)) 
                    {
                        if ($_POST['qty'] == 0) {
                            ...
                            $res = get_cart_total($this->data['user']->id, $order_item['product_variant_id']);
                        }
                    }
                }
            }
            ...
            $this->response['message'] = 'Item(s) Added Into Cart';
            $this->response['data'] = array();
            print_r(json_encode($this->response));
            return;                   
        }
    ...
    public function manage()
    {
        if ($this->data['is_logged_in']) {
            ...
            $this->db->select('sc.is_service_category');
            $this->db->from('product_variants as pv');
            $this->db->join('products p','pv.product_id=p.id');
            $this->db->join('super_categories sc', 'sc.id = p.super_category_id', 'left');
            $this->db->where('pv.id', $this->input->post('product_variant_id', true));
            $cp_info = $this->db->get();
            $cp_info = $cp_info->row_array();
            
            $current_cart_items     = get_cart_total($this->data['user']->id);
            if($current_cart_items['quantity'])
            {
                $service_product_text   = ($current_cart_items[0]['is_service_category']) ? 'products' : 'services';
                $check_product_service  = ($cp_info['is_service_category'] !=$current_cart_items[0]['is_service_category']) ? 1 : 0;
                
                if($check_product_service)
                {
                    $this->response['error'] = true;
                    $this->response['message'] = "Sorry, You need to order ".$service_product_text." seperately.";
                    $this->response['data'] = array();
                    print_r(json_encode($this->response));
                    return false;
                }
            ...
            if($this->input->post('current_plus', false))
            {
                $current_cart_items = get_cart_total($this->data['user']->id,  $this->input->post('product_variant_id', true));
                $current_qty = $current_cart_items['quantity'];
            }
            
            $data = array(
                'product_variant_id' => $this->input->post('product_variant_id', true),
                'qty' => $current_qty + $this->input->post('qty', true),
            ...
            if (!$this->cart_model->add_to_cart($data)) {
                ...
                $this->db->select('a.*, u.unit');
                $this->db->from('product_variants as a');
                $this->db->join('units as u','a.unit_id = u.id');
                $this->db->where('a.id', $_POST['product_variant_id']);
                $atr_info = $this->db->get();
                $atr_info = $atr_info->row_array();
                
                $volume = '';
                if($atr_info)
                {
                    $attribute_value_ids = $atr_info['attribute_value_ids'];
                    if($attribute_value_ids!=null)
                    {
                        $attribute_value_ids = explode(',', str_replace(' ','',$attribute_value_ids));
                    }
                    ...
                    if(!empty($attribute_value_ids))
                    {
                        ...
                        $volume = ($at_value['volume']*$_POST['qty']).' '.$at_value['unit'];
                    }
                    else
                    {
                        $volume = ($atr_info['total_weight']*$_POST['qty']).' '.$atr_info['unit'];
                    }
                }
                ...
                $this->response['data'] = [
                    'total_quantity' => ($current_qty + $_POST['qty'] == 0) ? '0' : strval($current_qty + $_POST['qty']),
                    'sub_total' => strval($res['sub_total']),
                    'total_mrp' => strval($res['total_mrp']),
                    ...
                    'items' => $items,
                    'volume'=> $volume,
                    'total_cart_qty' => array_sum(array_column($items,'qty')),
                ];
                print_r(json_encode($this->response));
                return false;
            }
    ...
    public function remove()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if ($this->cart_model->remove_from_cart($data)) {
                ...
                $this->response['total_cart_qty'] = $total_cart_qty;
                $this->response['error'] = false;
    ...
    public function get_user_cart()
    {
        if ($this->data['is_logged_in']) {
            ...
            $this->response['total_mrp'] =  $cart_total_response['total_mrp'];
    ...
    public function checkout()
    {
        if ($this->data['is_logged_in']) {
            ...
            $product_var_ids = array();
            if($cart)
            {
                foreach($cart as $c)
                {
                    $product_var_ids[] = $c['product_variant_id'];
                }
            }
            
            if(empty($product_var_ids))
            {
                redirect(base_url().'cart');
            }
            
            $this->db->distinct();
            $this->db->select('a.seller_id, d.company_name, d.logo, d.min_order_value');
            ...
            $this->data['wallet_balance'] = fetch_details('id=' . $this->data['user']->id, 'users', 'balance,mobile');
            
            $this->data['default_address'] = $this->address_model->get_address($this->data['user']->id, NULL, NULL, TRUE);
            $this->data['billing_address'] = $this->address_model->get_billing_address($this->data['user']->id, NULL, NULL, TRUE);
    ...
    public function place_order()
    {
        ...
        */
            ...
            if (!isset($_POST['billing_address_id']) || empty($_POST['billing_address_id'])) {
                $this->response['error'] = true;
                $this->response['message'] = "Billing address not found.";
                $this->response['data'] = array();
                print_r(json_encode($this->response));
                return false;
            }
        ...
        */
            ...
            $this->form_validation->set_rules('billing_address_id', 'Billing Address', 'trim|required|numeric|xss_clean', array('required' => 'Billing Address'));
            ...
            if (!$this->form_validation->run()) {
                ...
                for ($i=0; $i < count($product_variant_id); $i++) { 
                    ...
                    $is_allowed = fetch_details(['id' => $product_id[0]['product_id']],"products",'cod_allowed,name');
                ...
                $wallet_balance = fetch_details('id=' . $_POST['user_id'], 'users', 'balance');
                ...
                $qty_data = array();
                $i = 0;
                foreach($product_variant_id as $product_variant)
                {
                    $qty_data[$product_variant] = $quantity[$i];
                    $i++;
                }
                
                $this->db->distinct();
                $this->db->select('a.seller_id');
                $this->db->from('products as a');
                $this->db->join('product_variants as b','a.id = b.product_id');
                $this->db->where_in('b.id', $product_variant_id);
                $query = $this->db->get();
                $seller_ids   = $query->result_array();
                
                $this->db->select('b.id, a.seller_id');
                $this->db->from('products as a');
                $this->db->join('product_variants as b','a.id = b.product_id');
                $this->db->where_in('b.id', $product_variant_id);
                $query = $this->db->get();
                $prod_ids   = $query->result_array();
                                
                if($seller_ids)
                {                    
                    $seller_wise_products = array();
                    $seller_wise_qtys = array();
                    foreach($prod_ids as $prod_id)
                    {
                        $seller_wise_products[$prod_id['seller_id']][] = $prod_id['id'];
                        $seller_wise_qtys[$prod_id['seller_id']][] = $qty_data[$prod_id['id']];
                    }
                    ...
                    {
                        $product_ids = $seller_wise_products[$seller_id['seller_id']];
                        $quantitys   = $seller_wise_qtys[$seller_id['seller_id']];
                        
                        $order_data  = array();
                        $order_data['is_service_category']           = $_POST['is_service_category'];
                        $order_data['user_id']                       = $_POST['user_id'];
                        $order_data['mobile']                        = $_POST['mobile'];
                        $order_data['is_delivery_charge_returnable'] = $_POST['is_delivery_charge_returnable'];
                        $order_data['payment_method']                = $_POST['payment_method'] ;
                        $order_data['address_id']                    = $_POST['address_id'];
                        $order_data['billing_address_id']            = $_POST['billing_address_id'];
                        $order_data['delivery_date']                 = $_POST['delivery_date'];
                        $order_data['delivery_time']                 = $_POST['delivery_time'];
                        $order_data['notes']                         = $_POST['order_note'];
                        $order_data['product_variant_id']            = implode(',',$product_ids);
                        $order_data['quantity']                      = implode(',',$quantitys);
                        $order_data['promo_code']                    = ''; 
                        $order_data['is_wallet_used']                = 0;
                        $order_data['active_status']                 = 'received';
                        $order_data['last_updated']                  = date('Y-m-d H:i:s');
                        ...
                        $order_ids[] = '<strong>HC-A'.$res['order_id'].'<strong>';
                    ...
                    {
                        $order_msg = " Your order ids are ".implode(", ",$order_ids);
                    }
                    else
                    {
                        $order_msg = " Your order id is ".implode(", ",$order_ids);
                    }
                    
                    $this->response['error'] = false;
                    $this->response['message'] = "Your order has been placed successfully. ".$order_msg."<br/> <u><a class='btn btn-sm btn-primary' href='".base_url('my-account/orders')."'>View order details</a></u>";
    ...
    public function pre_payment_setup()
    {
        if ($this->data['is_logged_in']) {
            $_POST['user_id'] = $this->data['user']->id;
            $cart = get_cart_total($this->data['user']->id, false, '0', $_POST['address_id']);
            $wallet_balance = fetch_details('id=' . $this->data['user']->id, 'users', 'balance');
            $wallet_balance = $wallet_balance[0]['balance'];
