...
class Category extends CI_Controller
{
    ...
    {
        $inputFileType = 'Xlsx';
        $inputFileName = 'uploads/bio-pesticides2.xlsx';
        ...
        if($sheetData)
        {
            ...
            $i = 0;
            $x = 2;
            $image = 'uploads/media/2022/fertilizer.png';
            foreach($sheetData as $row)
            {
                if($i>0)
                {
                    $name   = strtolower($row['B']);
                    $name   = ucwords($name);
                    $row_array = array(
                                    'name'              => trim($name),
                                    'parent_id'         => trim($row['C']),
                                    'super_category_id' => trim($row['D']),
                                    'image'             => $image,
                                    'slug'              => create_unique_slug($name, 'categories'),
                                    'status'            => '1',
                                 );
                    $this->db->insert('categories', $row_array);
    ...
    /*public function import_insects()
    {
        $inputFileType = 'Xlsx';
        $inputFileName = 'uploads/bio-pesticides_insects.xlsx';
        ...
        if($sheetData)
        {
            ...
            foreach($sheetData as $row)
            {
                if($i>0)
                {
                    ...
                    $row_array = array(
                                    'name'              => trim($name),
                                    'sub_category_id'   => trim($row['C']),
                                    'super_category_id' => trim($row['D']),
                                    'image'             => $image,
                                    'slug'              => create_unique_slug($name, 'insect_pests'),
                                    'status'            => '1',
                                 );
                    $this->db->insert('insect_pests', $row_array);
