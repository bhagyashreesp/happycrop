...
class Product extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            redirect('admin/product/products/1', 'refresh');
    ...
    public function products($super_category_id = 0, $product_status = '')
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->data['main_page'] = TABLES . 'manage-product';
            $settings = get_settings('system_settings', true);
            ...
            $this->data['sellers'] = $this->db->select(' u.username as seller_name,u.id as seller_id,sd.category_ids,sd.id as seller_data_id, sd.company_name  ')
            ...
            $this->data['page_title'] = 'Manage '.$super_category_name.' Products';
            
            if($product_status == 1)
            {
                $this->data['page_title'] = 'Approved '.$super_category_name.' Products';
            }
            else if($product_status == 2)
            {
                $this->data['page_title'] = 'Unapproved '.$super_category_name.' Products';
            }
            else if($product_status == 0 && $product_status!='')
            {
                $this->data['page_title'] = 'Deactivated '.$super_category_name.' Products';
            }
    ...
    public function new_product()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $sp_page_id = isset($_GET['super_category_id']) ? $_GET['super_category_id'] : 1;
            
            if(file_exists('application/views/admin/pages/'.FORMS . 'new_product_'.$sp_page_id.'.php'))
            {
                $this->data['main_page'] = FORMS . 'new_product_'.$sp_page_id;
            }
            else
            {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                if (!empty($product_details)) {
                    ...
                    redirect('admin/product/new_product', 'refresh');
                }
            }
    ...
    public function add_new_product()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['redirect_url'] = base_url('admin/product/products/'.$super_category_id);
    ...
    public function get_product_data($super_category_id = 0, $product_status = '')
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
