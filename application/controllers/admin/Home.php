...
class Home extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $orders_count['total_sales']        = admin_orders_total();
            ...
            $orders_count['issue_raised_orders']= admin_orders_count(array("complaint","complaint_msg"));
            $orders_count['cancelled_orders']   = admin_orders_count(array("cancelled"));
            $orders_count['delivered_orders']   = admin_orders_count(array("delivered","send_mfg_payment_ack","send_mfg_payment_confirmation"));
            
            $this->data['status_counts'] = $orders_count;
            
            $this->data['mfg_total']            = mfg_total();
            $this->data['approved_mfg']         = mfg_total(1);
            $this->data['unapproved_mfg']       = mfg_total(2);
            
            $this->data['retailer_total']       = retailer_total();
            $this->data['approved_retailer']    = retailer_total(1);
            $this->data['unapproved_retailer']  = retailer_total(2);
    ...
    public function fetch_sales()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $week_res = $this->db->select("DATE_FORMAT(date_added, '%d-%b') as date, SUM(final_total) as total_sale")
                ...
                ->order_by('date_added','asc')->group_by('day(date_added)')->get('`orders`')->result_array();
            ...
            $day_res = $this->db->select("DAY(date_added) as date, SUM(final_total) as total_sale")
                ...
                ->group_by('day(date_added)')->order_by('date_added','asc')->get('`orders`')->result_array();
            $day_wise_sales['total_sale'] = array_map('intval', array_column($day_res, 'total_sale'));
            $day_wise_sales['day'] = array_column($day_res, 'date');
