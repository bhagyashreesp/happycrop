...
class Product extends CI_Controller
{
    ...
    public function products($super_category_id = 0)
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            $this->data['super_category_info'] = $scat;
            $this->data['meta_description'] = 'Product Management |' . $settings['app_name'];
            $this->data['categories'] = json_decode(json_encode($this->category_model->get_seller_categories($seller_id)), 1);
            $this->load->view('seller/template', $this->data);
        } else {
    ...
    public function new_product()
    {
        if($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            $this->db->select('a.verify_field');
            $this->db->from('super_categories a');
            $this->db->where(['a.status' => 1]);
            $this->db->where(['a.id' => $sp_page_id]);
            $scat = $this->db->get();
            $scat = $scat->row_array();
            
            $verify_field = $scat['verify_field'];
            ...
            if($verify_field!='')
            {
                ...
                {
                    if(file_exists('application/views/seller/pages/'.FORMS . 'new_product_'.$sp_page_id.'.php'))
                    {
                    ...
                    $this->data['super_category_id'] = isset($_GET['super_category_id']) ? $_GET['super_category_id'] : 1;
                ...
                {
                    redirect('seller/product/products/'.$sp_page_id, 'refresh');
                }
            }
            else 
            {
