...
class Auth extends CI_Controller
{
    ...
    public function create_custom_seller()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if (update_details(['active' => 1], [$identity_column => $identity], 'users')) {
                ...
                $user = fetch_details(['id' => $user_id[0]['id']], 'users');
                ...
                /*if($user[0]['email']!='')
                {
                    ...
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>Expanded Market Reach:</b> By joining Happycrop, manufacturers gain access to a vast network of retailers, transcending geographical limitations.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>Wide Product Range:</b> Showcase your agri inputs to a diverse audience of retailers, expanding your market presence.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>Extended Market Visibility:</b> Gain exposure to a network of potential buyers, leading to enhanced business growth.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>Effortless Transactions:</b> Seamlessly manage orders, track deliveries, and handle invoices in one streamlined platform.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>Cost Efficiency:</b> By cutting out unnecessary intermediaries, you\'ll enjoy enhanced profit margins and better control over pricing.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>User-Friendly Interface:</b> Navigate our intuitive dashboard to list products, track sales, and stay connected.</p>';
                    $html_text .= '<p style="margin-top:20px;">Your Next Steps:</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>1. Complete Your Profile:</b> Log in to your Happycrop account and fill in your profile details. An informative profile helps buyers connect with you better.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>2. List Your Products:</b> Showcase your top-quality agri inputs by listing them on our platform. Accurate product details attract potential buyers.</p>';
                    $html_text .= '<p style="margin-bottom:0px;margin-top: 2px;"><b>3. Engage with Buyers:</b> Respond promptly to inquiries from retailers interested in your products. Building strong connections can lead to valuable partnerships.</p>';
                ...
                $insert_id = $this->Seller_model->add_custom_seller($data);
                if (!empty($insert_id)) {
                    
                    if ($insert_id) {
                        $group_id = $this->ion_auth->get_users_groups($insert_id)->row()->id;
                        $this->ion_auth->remove_from_group($group_id, $insert_id);
                        $this->ion_auth->add_to_group('4', $insert_id);
                    }
                    ...
                    if($this->ion_auth->login($identity, $password, $remember)) {
                        ...
                        if(!$user[0]['mfg_no'])
                        {
                            ...
                            $this->db->update('users', $_no_data2, array('id'=>$user[0]['id']));
                        }
                        
                        $this->response['redirect_to'] = base_url('register/step4/1');
                        $this->response['error'] = false;
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
                        $this->response['csrfHash'] = $this->security->get_csrf_hash();
                        $this->response['message'] = 'Your are registered Successfully.';
                        print_r(json_encode($this->response));    
                    }
                    ...
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = "Your data was not added";
                    print_r(json_encode($this->response));
                }
            }
        }
